...
class Model(object):
    ...
    def build_fcn_net(self, inp, use_dice = False):
        ...
        dnn1 = add_output_scaling_factor_v2(dnn1, name='fcn_sf1')
        ...
        dnn2 = add_output_scaling_factor_v2(dnn2, name='fcn_sf2')
        dnn3 = tf.layers.dense(dnn2, 2, activation=None, name='f3')
        self.y_hat = tf.nn.softmax(dnn3) + 0.00000001
        self.y_hat = tf.identity(self.y_hat, name="output")
    ...
    def auxiliary_net(self, in_, stag='auxiliary_net'):
        ...
        dnn1 = add_output_scaling_factor_v2(dnn1, name='sf1_aux_' + stag, reuse=tf.AUTO_REUSE)
        dnn2 = tf.layers.dense(dnn1, 50, activation=None, name='f2' + stag, reuse=tf.AUTO_REUSE)
        dnn2 = tf.nn.sigmoid(dnn2)
        dnn2 = add_output_scaling_factor_v2(dnn2, name='sf2_aux_' + stag, reuse=tf.AUTO_REUSE)
        dnn3 = tf.layers.dense(dnn2, 2, activation=None, name='f3' + stag, reuse=tf.AUTO_REUSE)
        self.aux_logits = dnn3
        y_hat = tf.nn.softmax(dnn3) + 0.00000001
        return y_hat

    def train_flops(self, sess, inps):
        ...
        if self.use_negsampling:
            ...
            print("train_flops: %s"%(str(flops.total_float_ops)))
...
class Model_DIN_V2_Gru_Vec_attGru_Neg(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True, gamma=0.0):
        ...
        with tf.name_scope('rnn_1'):
            rnn_outputs, _ = dynamic_rnn(GRUCell(HIDDEN_SIZE), inputs=self.item_his_eb,
                                         sequence_length=self.seq_len_ph, dtype=tf.float32,
                                         scope="gru1")
            rnn_outputs = add_output_scaling_factor_v2(rnn_outputs, name='gru1')
            tf.summary.histogram('GRU_outputs', rnn_outputs)
        ...
        with tf.name_scope('rnn_2'):
            rnn_outputs2, final_state2 = dynamic_rnn(VecAttGRUCell(HIDDEN_SIZE), inputs=rnn_outputs,
                                                     ...
                                                     sequence_length=self.seq_len_ph, dtype=tf.float32,
                                                     scope="gru2")
            #rnn_outputs2, final_state2 = dynamic_rnn(GRUCell(HIDDEN_SIZE), inputs=rnn_outputs,
                                                     #sequence_length=self.seq_len_ph, dtype=tf.float32,
                                                     #scope="gru2")
            final_state2 = add_output_scaling_factor_v2(final_state2, name='gru2') 
            tf.summary.histogram('GRU2_Final_State', final_state2)
    ...
    def scaling_factor(self):
        self.scalars = [v for v in tf.global_variables() if "scale_factor_" in v.name and 'Adam' not in v.name]
        self.scalar_names = [v.name for v in self.scalars]
        
    def flops(self, sess, inps):
        run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)
        run_metadata = tf.RunMetadata()
        if self.use_negsampling:
            ...
            cmd='op', run_meta=run_metadata,
            options=tf.profiler.ProfileOptionBuilder.float_operation())
            print("inference_flops: %s"%(str(flops.total_float_ops)))
            return y_hat
        else:
            y_hat = sess.run([self.y_hat], feed_dict={
                self.uid_batch_ph: inps[0],
    ...
    def train(self, sess, inps):
        if self.use_negsampling:
            loss, accuracy, aux_loss, _, regularizer_loss, current_cost, scalars = sess.run([self.loss, self.accuracy, self.aux_loss, self.optimizer, self.regularizer_loss, self.current_cost, self.scalars], feed_dict={
                self.uid_batch_ph: inps[0],
                self.mid_batch_ph: inps[1],
                self.cat_batch_ph: inps[2],
                self.mid_his_batch_ph: inps[3],
            ...
            return loss, accuracy, aux_loss, regularizer_loss, current_cost, scalars
        else:
            loss, accuracy, _, regularizer_loss, current_cost ,scalars = sess.run([self.loss, self.accuracy, self.optimizer, self.regularizer_loss, self.current_cost, self.scalars], feed_dict={
                self.uid_batch_ph: inps[0],
                self.mid_batch_ph: inps[1],
                self.cat_batch_ph: inps[2],
                self.mid_his_batch_ph: inps[3],
            ...
            return loss, accuracy, 0, regularizer_loss, current_cost, scalars 
    ...
    def build_fcn_net(self, inp, use_dice = False):
        ...
        with tf.name_scope('Metrics'):
            ...
            network_regularizer = flop_regularizer.GammaFlopsRegularizer(
                    output_boundary = [dnn3.op],
                    input_boundary = [inp.op],
                    gamma_threshold=1e-3
                    )
            regularization_strength = 8e-7
            self.regularizer_loss = (network_regularizer.get_regularization_term() * regularization_strength)
            self.current_cost = network_regularizer.get_cost()

            self.loss = ctr_loss
            if self.use_negsampling:
                self.loss += self.aux_loss
            tf.summary.scalar('loss', self.loss)
            self.optimizer = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss + self.regularizer_loss)
            # Accuracy metric
            self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.round(self.y_hat), self.target_ph), tf.float32))
            tf.summary.scalar('accuracy', self.accuracy)
