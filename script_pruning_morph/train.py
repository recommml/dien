...
def train(
    ...
    model_path = "dnn_save_path_morph/ckpt_noshuff" + model_type + str(seed)
    best_model_path = "dnn_best_model_morph/ckpt_noshuff" + model_type + str(seed)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for itr in range(3):
            ...
            accuracy_sum = 0.
            aux_loss_sum = 0.
            regularizer_loss_sum = 0. 
            current_cost_sum = 0.
            for src, tgt in train_data:
                uids, mids, cats, mid_his, cat_his, mid_mask, target, sl, noclk_mids, noclk_cats = prepare_data(src, tgt, maxlen, return_neg=True)
                if model_type == 'DIEN':
                    loss, acc, aux_loss, regularizer_loss, current_cost, scalars = model.train(sess, [uids, mids, cats, mid_his, cat_his, mid_mask, target, sl, lr, noclk_mids, noclk_cats])
                    loss_sum += loss
                    accuracy_sum += acc
                    aux_loss_sum += aux_loss
                    regularizer_loss_sum += regularizer_loss
                    current_cost_sum += current_cost
                    iter += 1
                    sys.stdout.flush()
                    if (iter % test_iter) == 0:
                        ...
                        print('                                                                                          test_auc: %.4f ----test_loss: %.4f ---- test_accuracy: %.4f ---- test_aux_loss: %.4f' % eval(sess, test_data, model, best_model_path))
                        print('regularizer_loss: %.4f ---- current_cost: %.4f' % (regularizer_loss_sum / test_iter, current_cost_sum / test_iter))
                        loss_sum = 0.0
                        accuracy_sum = 0.0
                        aux_loss_sum = 0.0
                        regularizer_loss_sum = 0.0
                        current_cost_sum = 0.0
                        #_ = model.flops(sess, [uids, mids, cats, mid_his, cat_his, mid_mask, target, sl, lr, noclk_mids, noclk_cats])
                        for i in range(len(scalars)):
                            each = scalars[i]
                            name = model.scalar_names[i]
                            dim = each.shape[0]
                            zeros = numpy.count_nonzero(numpy.abs(each) <= 0.0001)
                            l1_norm = numpy.mean(numpy.abs(each))
                            print("name:%s, dim: %d, zeros: %d, sparsity: %f, l1_norm: %f"% (name, dim, zeros, float(zeros)/dim, l1_norm))
...
def test(
    ...
    model_path = "dnn_best_model_morph/ckpt_noshuff" + model_type + str(seed)
    gpu_options = tf.GPUOptions(allow_growth=True)
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
