...
class Model(object):
    ...
    def build_fcn_net(self, inp, use_dice = False):
        ...
        dnn1 = tf.layers.dense(bn1, 170, activation=None, name='f1')
        if use_dice:
            dnn1 = dice(dnn1, name='dice_1')
        else:
            dnn1 = prelu(dnn1, 'prelu1')
        dnn1 = add_output_scaling_factor(dnn1, 'fcn_sf1')

        dnn2 = tf.layers.dense(dnn1, 70, activation=None, name='f2')
        if use_dice:
            dnn2 = dice(dnn2, name='dice_2')
        else:
            dnn2 = prelu(dnn2, 'prelu2')
        dnn2 = add_output_scaling_factor(dnn2, 'fcn_sf2')
    ...
    def auxiliary_net(self, in_, stag='auxiliary_net'):
        ...
        dnn1 = tf.layers.dense(bn1, 40, activation=None, name='f1' + stag, reuse=tf.AUTO_REUSE)
        dnn1 = tf.nn.sigmoid(dnn1)
        dnn1 = add_output_scaling_factor(dnn1, 'sf1_aux_' + stag, reuse=tf.AUTO_REUSE)
        dnn2 = tf.layers.dense(dnn1, 30, activation=None, name='f2' + stag, reuse=tf.AUTO_REUSE)
        dnn2 = tf.nn.sigmoid(dnn2)
        dnn2 = add_output_scaling_factor(dnn2, 'sf2_aux_' + stag, reuse=tf.AUTO_REUSE)
...
class Model_DIN_V2_Gru_Vec_attGru_Neg(Model):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, use_negsampling=True, gamma=0.0):
        ...
        with tf.name_scope('rnn_1'):
            rnn_outputs, _ = dynamic_rnn(GRUCell(HIDDEN_SIZE), inputs=self.item_his_eb,
                                         sequence_length=self.seq_len_ph, dtype=tf.float32,
                                         scope="gru1")
            rnn_outputs = add_output_scaling_factor(rnn_outputs, 'gru1') 
            tf.summary.histogram('GRU_outputs', rnn_outputs)
        ...
        with tf.name_scope('rnn_2'):
            ...
            rnn_outputs2, final_state2 = dynamic_rnn(GRUCell(HIDDEN_SIZE), inputs=rnn_outputs,
                                                     sequence_length=self.seq_len_ph, dtype=tf.float32,
                                                     scope="gru2")
            final_state2 = add_output_scaling_factor(final_state2, 'gru2') 
            tf.summary.histogram('GRU2_Final_State', final_state2)

        inp = tf.concat([self.uid_batch_embedded, self.item_eb, self.item_his_eb_sum, self.item_eb * self.item_his_eb_sum, final_state2], 1)
        self.build_fcn_net(inp, use_dice=True)
        self.scaling_factor_updater()
        
    def scaling_factor_updater(self):
        scalars = [v for v in tf.global_variables() if "scale_factor_" in v.name and 'Adam' not in v.name]
        modified_scalars = [soft_thresholding(x, self.lr * self.gamma) for x in scalars] 
        self.scalars = []
        for ori, modi in zip(scalars, modified_scalars):
            self.scalars.append(ori.assign(modi))
        self.scalar_names = [v.name for v in scalars]
    ...
    def train_flops(self, sess, inps):
        ...
        run_metadata = tf.RunMetadata()
        if self.use_negsampling:
            loss, accuracy, aux_loss, _, scalars = sess.run([self.loss, self.accuracy, self.aux_loss, self.optimizer, self.scalars], feed_dict={
                self.uid_batch_ph: inps[0],
                self.mid_batch_ph: inps[1],
                self.cat_batch_ph: inps[2],
                self.mid_his_batch_ph: inps[3],
            ...
            return loss, accuracy, aux_loss, scalars
        else:
            loss, accuracy, _, scalars = sess.run([self.loss, self.accuracy, self.optimizer, self.scalars], feed_dict={
                self.uid_batch_ph: inps[0],
                self.mid_batch_ph: inps[1],
                self.cat_batch_ph: inps[2],
                self.mid_his_batch_ph: inps[3],
            ...
            options=tf.profiler.ProfileOptionBuilder.float_operation())
            print("train_flops: %s"%(str(flops.total_float_ops)))
            return loss, accuracy, 0, scalars
