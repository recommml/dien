...
def soft_thresholding(x, gamma):
    ...
    y = tf.maximum(0.0, tf.abs(x) - gamma)
    return tf.sign(x) * y
    
def add_output_scaling_factor(_x, scope='', reuse=False):
    with tf.variable_scope(name_or_scope=scope, default_name="scaling_factor", reuse=reuse):
        _lambda = tf.get_variable("scale_factor_"+scope, shape=_x.get_shape()[-1],
                                 dtype=_x.dtype, initializer=tf.constant_initializer(1.0))
        return  _lambda * _x

def sparse_factor_func(x, _lambda):
    return x * _lambda

def add_output_scaling_factor_v2(_x, name='', reuse=False):
    with tf.variable_scope(name_or_scope=name, default_name="scaling_factor", reuse=reuse):
        _lambda = tf.get_variable("scale_factor_"+name, shape=_x.get_shape()[-1],
                                 dtype=_x.dtype, initializer=tf.constant_initializer(1.0))
        g = tf.get_default_graph()
        with g.gradient_override_map({"PyFunc": "Mul"}):
            y = tf.py_func(sparse_factor_func, [_x, _lambda], [_x.dtype])
            y[0].set_shape(_x.get_shape())
        return y[0] 
...
def din_fcn_attention(query, facts, attention_size, mask, stag='null', mode='SUM', softmax_stag=1, time_major=False, return_alphas=False, forCnn=False):
    ...
    query = prelu(query)
    query = add_output_scaling_factor(query, 'f1_' + stag)
    queries = tf.tile(query, [1, tf.shape(facts)[1]])
    queries = tf.reshape(queries, tf.shape(facts))
    din_all = tf.concat([queries, facts, queries-facts, queries*facts], axis=-1)
    d_layer_1_all = tf.layers.dense(din_all, 80, activation=tf.nn.sigmoid, name='f1_att' + stag)
    d_layer_1_all = add_output_scaling_factor(d_layer_1_all, 'f1_att_' + stag)
    d_layer_2_all = tf.layers.dense(d_layer_1_all, 40, activation=tf.nn.sigmoid, name='f2_att' + stag)
    d_layer_2_all = add_output_scaling_factor(d_layer_2_all, 'f2_att_' + stag)
    d_layer_3_all = tf.layers.dense(d_layer_2_all, 1, activation=None, name='f3_att' + stag)
    d_layer_3_all = tf.reshape(d_layer_3_all, [-1, 1, tf.shape(facts)[1]])
    scores = d_layer_3_all
    # Mask
