...
def train(
    ...
    model_path = "dnn_save_path_v2/ckpt_noshuff" + model_type + str(seed)
    best_model_path = "dnn_best_model_v2/ckpt_noshuff" + model_type + str(seed)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for itr in range(5):
            ...
            for src, tgt in train_data:
                ...
                if model_type == 'DIEN':
                    if(iter % test_iter != 0):
                        loss, acc, aux_loss, scalars = model.train(sess, [uids, mids, cats, mid_his, cat_his, mid_mask, target, sl, lr, noclk_mids, noclk_cats])
                        loss_sum += loss
                        accuracy_sum += acc
                        aux_loss_sum += aux_loss
                        iter += 1
                        ...
                        loss, acc, aux_loss, scalars = model.train_flops(sess, [uids, mids, cats, mid_his, cat_his, mid_mask, target, sl, lr, noclk_mids, noclk_cats])
                        loss_sum += loss
                        accuracy_sum += acc
                        aux_loss_sum += aux_loss
                        iter += 1
...
def test(
    ...
    model_path = "dnn_best_model_v2/ckpt_noshuff" + model_type + str(seed)
    gpu_options = tf.GPUOptions(allow_growth=True)
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
